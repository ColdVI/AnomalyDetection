...
- `project_support/`: önceki Makefile ve eski ML dönemine ait proje çıktıları
  (`output.txt` vb.);
- `src/ingestion`, `src/silver`, `src/gold`, `src/processing`: yalnız eski
  ALFA/UAV/RFLY akışına ait parser ve pipeline parçaları.
...
**2026-07-10 düzeltme:** `project_support/Dashboard` ve `project_support/individual`
buradan geri çıkarıldı — bunlar ML deneyleri değil, takım arkadaşlarının (Yusuf,
Metehan) kendi ayrı teslimatları; "legacy" etiketi altına konmaları yanlış
çerçevelemeydi. Güncel konumları: kök dizinde `Dashboard/` ve `individual/`.
