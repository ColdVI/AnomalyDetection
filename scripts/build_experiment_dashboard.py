...
TEMPLATE = r"""<title>Deney Kayit Paneli</title>
<style>
:root {
  --bg: #f5f3ee;
  --bg-elevated: #ffffff;
...
<div class="page">
  <header class="top">
    ...
    <p>ADR gunlugunden ve ADS-B contextual_physics_v1 hattindan cikarilan tum deney/model sonuclari — filtrelenebilir tek liste.
    Egrileri gorsellestiren panel icin: <a href="adsb_contextual_physics_v1_burden_dashboard.html">dogal alarm yuku paneli</a>.</p>
  </header>
...
const FILE_REF_RE = /\b((?:docs|artifacts|scripts|configs)\/[\w./-]+\.(?:html|json|md))\b/g;
function autolinkFileRefs(text) {
  if (!text) return text;
  return text.replace(FILE_REF_RE, (match, path) => {
    const href = path.startsWith("docs/") ? path.slice("docs/".length) : "../" + path;
  ...
  return `
    <div class="card" id="card-${idx}">
      ...
      <div class="card-details">
        ...
        ${gatesHtml}
        ${metricsHtml}
        ${e.notes ? `<h4>Notlar</h4><div class="notes">${autolinkFileRefs(e.notes)}</div>` : ""}
      </div>
    </div>`;
}
