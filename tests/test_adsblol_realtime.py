...
from src.ingestion.adsblol_consumer import flush_batch_to_bronze, land_batch_raw, turkey_rows


def test_turkey_rows_filters_by_bbox_and_handles_missing_coords():
    batch = [
        {"hex": "tc001", "lat": 39.0, "lon": 35.0},  # inside
        {"hex": "ab002", "lat": 51.0, "lon": -10.0},  # outside
        {"hex": "noposition"},  # mode_s / no position yet -- must not crash
    ]

    result = turkey_rows(batch)

    assert [entry["hex"] for entry in result] == ["tc001"]


def test_flush_batch_to_bronze_writes_only_turkey_rows_with_provenance(fake_minio_client):
    batch = [
    ...
    assert list(df["hex"]) == ["tc001"]
    assert df.loc[0, "_source_type"] == "adsblol_rt"
    assert df.loc[0, "_source_file"] == "uav.raw.states"


def test_flush_batch_to_bronze_returns_none_when_nothing_in_turkey(fake_minio_client):
    batch = [{"hex": "ab002", "lat": 51.0, "lon": -10.0}]
    uri = flush_batch_to_bronze(batch, source_file="uav.raw.states", client=fake_minio_client)
    assert uri is None
...
def test_land_batch_raw_writes_every_message_unfiltered(fake_minio_client):
    batch = [
        ...
        {"hex": "ab002", "lat": 51.0, "lon": -10.0},  # outside Turkey, but landing keeps it
    ]
