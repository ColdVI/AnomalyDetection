...
- Bucket: `uav-history`, retention 7 gün (script: `update_retention.py` ile değiştirilebilir, `--days 0` = sınırsız).
...
Tek kaynak (adsb.lol) olsa da EKİP bağımsız çalışıyor: sen dashboard, Yusuf MinIO arşivi, Anıl/model ekibi anomali tespiti — hepsi aynı `uav.flights` topic'ini kendi `group.id`'siyle bağımsız okuyabilir, birbirinin koduna dokunmadan. Kafka'nın gerekçesi hacim değil, **organizasyonel ayrışma**.
...
adsb.lol API (15sn'de bir, lat=39/lon=35/radius=500nm)
        |
        v
uav_producer.py  --(Kafka: uav.flights, 3 partition)-->  Kafka broker (C:\kafka, KRaft degil Zookeeper modu)
                                                                      |
                                                    dashboard_consumer.py (group=dashboard-consumer)
                                                        ...
                                                        v                           v
                                                     Redis                     InfluxDB
                                                  (canli state,              (bucket=uav-history,
...
Kafka (uav.flights) --> Yusuf'un MinIO bronze consumer'i (henuz yazilmadi, plan var)
Kafka (uav.flights) --> Model ekibinin anomali consumer'i (henuz yazilmadi)
                                  |
                                  v
                          Kafka (uav.alerts) --> dashboard_consumer.py otomatik yakalar
...
| `uav_producer.py` | adsb.lol'den çekip Kafka'ya yazan tek iş: fetch + parse + produce |
...
| `KAFKA_SCHEMA.md` | Ekip için: `uav.flights`/`uav.alerts` topic şema sözleşmesi |
...
InfluxDB            : {proje}\influxdb\influxd.exe, bucket=uav-history, 7 gun retention
...
FastAPI portu       : 8000
Dash portu          : 8050
Kafka topics        : uav.flights (3 partition), uav.alerts (1 partition)
...
- Yusuf'un Kafka consumer'ı (uav.flights → MinIO bronze JSONL) **tam olarak** bizim `KAFKA_SCHEMA.md`'de öngördüğümüz "arkadaşların kendi group.id'siyle bağlanabilir" senaryosu — entegrasyon noktası hazır, henüz gerçekleşmedi.
...
- [ ] Model ekibi henüz `uav.alerts` topic'ine hiçbir şey yazmıyor — alarm sistemi altyapısı hazır ama şu an boş.
...
4. {proje}'de:       python uav_producer.py                 (kendi terminali)
