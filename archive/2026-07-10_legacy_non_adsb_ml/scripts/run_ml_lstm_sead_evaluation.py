"""ML-16 Kol L: SEAD LSTM-AE retrained on refreshed splits, wired into the official
ml14/ml15 fusion + decision-layer evaluation harness (docs/ML16_KOL_L_LSTM_SEAD_PLAN.md).

Note: "ML-17" is reserved elsewhere in this repo for the blind-holdout endgame phase and
is NOT this phase -- this is the third arm ("Kol L") of ML-16
(docs/ML16_OZ_KOSULLU_DEDEKTOR_PLAN.md covers Kol F/Kol T).

Reuses, unmodified:
  - src/ml/models/lstm_autoencoder.py (LSTMAutoencoder, train_lstm_autoencoder,
    reconstruction_scores, masked_mse, AE_FEATURES/WINDOW/STRIDE)
  ...
  - scripts/run_ml8a_temporal_boosting.py::_align_score (the established, already-in-repo
    LSTM window-score -> arbitrary-timestamp causal alignment convention, merge_asof
    direction="backward")
...
LSTM_WINDOW = WINDOW[SOURCE]
LSTM_STRIDE = STRIDE[SOURCE]
MAX_GAP_S = 2.0

# (a)/(b)/(c) — the only three variants registered in docs/ML16_KOL_L_LSTM_SEAD_PLAN.md §1.
SCORE_SOURCES = ("lstm_recon", "lstm_ml14_fusion", "lstm_itki_fusion")
...
def _lstm_sequence(scaled: pd.DataFrame, raw: pd.DataFrame) -> pd.DataFrame:
...
def _train_lstm(sequence: pd.DataFrame, train_ids: set[str], val_ids: set[str], seed: int):
    ...
    if not len(x_train) or not len(x_val):
        raise RuntimeError("SEAD LSTM train/val pencereleri bos (train veya val ucus seti cok kisa)")
    model, training = train_lstm_autoencoder(
        LSTMAutoencoder(len(AE_COLS)), x_train, m_train, x_val, m_val, seed=seed,
    )
...
def _gate_b(metrics: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "rule": "any of {lstm_recon, lstm_ml14_fusion, lstm_itki_fusion} x "
...
def run(run_name: str, split_names: tuple[str, ...] | None = None) -> Path:
    ...
    if development & holdout:
        raise AssertionError("Blind holdout entered the ML-16 Kol L development request")
    ...
    output = ROOT / "artifacts/ml_lstm_sead/uav_sead" / run_name
    ...
    for split_name, split in folds.items():
        ...
        model, training, window_counts = _train_lstm(sequence, parts["train"], parts["val"], seed)
        history = training.pop("history")
        write_training_log(history, SOURCE, "ml16_kol_l_lstm_sead", f"{run_id}_{split_name}")
        ...
        scored["lstm_recon_raw"] = _align_score(endpoints, window_scores, "lstm_recon_raw")

        val_mask = scored["source_id"].isin(parts["val"]).to_numpy()
        scored["lstm_recon"] = empirical_probability(
            scored.loc[val_mask, "lstm_recon_raw"].to_numpy(dtype=float),
            scored["lstm_recon_raw"].to_numpy(dtype=float),
        )
        scored["lstm_ml14_fusion"] = max_score_fusion(scored, ["lstm_recon", "ml14_fusion"])
        scored["lstm_itki_fusion"] = max_score_fusion(scored, ["lstm_recon", "itki_komutu"])

        test_mask = scored["source_id"].isin(parts["test"]).to_numpy()
        coverage = {
            "split": split_name,
            "val_row_finite_fraction": float(np.isfinite(
                scored.loc[val_mask, "lstm_recon_raw"].to_numpy(dtype=float)).mean()),
            "test_row_finite_fraction": float(np.isfinite(
                scored.loc[test_mask, "lstm_recon_raw"].to_numpy(dtype=float)).mean()),
            **window_counts,
        }
        ...
        save_torch_checkpoint(model, models_dir / "lstm_ae.pt", metadata={
            "source": SOURCE,
        ...
        (split_dir / "lstm_coverage.json").write_text(json.dumps(coverage, indent=2), encoding="utf-8")
    ...
    artifact_manifest = {
        ...
        "stage": "ML-16 Kol L SEAD LSTM-AE retrain + official fusion/decision evaluation",
        "source": SOURCE,
        "plan": "docs/ML16_KOL_L_LSTM_SEAD_PLAN.md",
        ...
        "gps_health_column_coverage_note": (
            ...
            "masked_mse/build_windows mask channel absorbs it (see docs/ML16_KOL_L_LSTM_SEAD_PLAN.md sec2)."
        ),
...
def main() -> None:
    ...
    print(f"ML-16 Kol L artifact: {output}")
