...
- İlk güvenlik tavanı: `100` epoch. Bir rotasyon bu tavana ulaşıp en iyi epoch'u
  son patience penceresindeyse convergence sayılmaz; outer metriğe bakmadan aynı
  deterministik eğitim `500` tavana uzatılır. Aynı koşul 500'de de oluşursa son
  güvenlik tavanı `2000` olur. Bu tavanlar hiperparametre seçimi değil, yalnız
  early-stopping'in çalışmasına izin veren önceden sabitlenmiş kaynak sınırlarıdır.
...
sayısı yine altıdır. R4 sonucu başarısız olursa yeni LR/patience taraması
yapılmaz.
