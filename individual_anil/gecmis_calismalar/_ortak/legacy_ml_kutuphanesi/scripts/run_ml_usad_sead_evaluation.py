"""ML-16 Kol U: SEAD USAD (adversarial twin-autoencoder) wired into the official
ml14/ml15 fusion + decision-layer evaluation harness (docs/ML16_KOL_U_USAD_SEAD_PLAN.md).

Reuses, unmodified:
  - src/ml/models/usad.py (USAD, train_usad, usad_reconstruction_scores,
    masked_mse re-exported from lstm_autoencoder.py)
  - src/ml/models/lstm_autoencoder.py (AE_FEATURES/WINDOW/STRIDE -- SAME columns/window/
    stride as the LSTM-AE/Dense-AE families, for a fair comparison; no new windowing config)
...
This file mirrors scripts/run_ml_lstm_sead_evaluation.py / run_ml_dense_ae_sead_evaluation.py's
structure closely (same evaluation harness, different model family) -- intentional, not
...
from src.ml.models.usad import USAD, train_usad, usad_reconstruction_scores
...
USAD_WINDOW = WINDOW[SOURCE]
USAD_STRIDE = STRIDE[SOURCE]
MAX_GAP_S = 2.0

# (a)/(b)/(c) -- the only three variants registered in docs/ML16_KOL_U_USAD_SEAD_PLAN.md SS1.
SCORE_SOURCES = ("usad_score", "usad_ml14_fusion", "usad_itki_fusion")
...
def _usad_sequence(scaled: pd.DataFrame, raw: pd.DataFrame) -> pd.DataFrame:
...
def _train_usad(sequence: pd.DataFrame, train_ids: set[str], val_ids: set[str], seed: int):
    x_train, m_train, meta_train = build_windows(
        sequence[sequence["source_id"].isin(train_ids)], AE_COLS,
        window=USAD_WINDOW, stride=USAD_STRIDE, max_gap_s=MAX_GAP_S,
    )
    ...
    if not len(x_train) or not len(x_val):
        raise RuntimeError("SEAD USAD train/val pencereleri bos (train veya val ucus seti cok kisa)")
    model, training = train_usad(
        USAD(USAD_WINDOW, len(AE_COLS)), x_train, m_train, x_val, m_val, seed=seed,
    )
...
def _score_usad(model, sequence: pd.DataFrame, score_ids: set[str]) -> pd.DataFrame:
    x, mask, meta = build_windows(
        sequence[sequence["source_id"].isin(score_ids)], AE_COLS,
        window=USAD_WINDOW, stride=USAD_STRIDE, max_gap_s=MAX_GAP_S,
    )
    if not len(x):
        raise RuntimeError("SEAD USAD skorlama pencereleri bos")
    raw_scores = usad_reconstruction_scores(model, x, mask)
    return pd.DataFrame({
        "source_id": meta["source_id"].to_numpy(),
        "t_rel_s": meta["t_end"].to_numpy(dtype=float),
        "usad_score_raw": raw_scores,
    })
...
def _gate_b(metrics: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "rule": "any of {usad_score, usad_ml14_fusion, usad_itki_fusion} x "
...
def run(run_name: str, split_names: tuple[str, ...] | None = None) -> Path:
    ...
    if development & holdout:
        raise AssertionError("Blind holdout entered the ML-16 Kol U development request")
    ...
    output = ROOT / "artifacts/ml_usad_sead/uav_sead" / run_name
    ...
    for split_name, split in folds.items():
        ...
        sequence = _usad_sequence(scaled, raw)
        model, training, window_counts = _train_usad(sequence, parts["train"], parts["val"], seed)
        history = training.pop("history")
        write_training_log(history, SOURCE, "ml16_kol_u_usad_sead", f"{run_id}_{split_name}")

        score_ids = parts["val"] | parts["test"]
        window_scores = _score_usad(model, sequence, score_ids)

        endpoints = scored[["source_id", "t_rel_s"]].reset_index(drop=True)
        scored["usad_score_raw"] = _align_score(endpoints, window_scores, "usad_score_raw")

        val_mask = scored["source_id"].isin(parts["val"]).to_numpy()
        scored["usad_score"] = empirical_probability(
            scored.loc[val_mask, "usad_score_raw"].to_numpy(dtype=float),
            scored["usad_score_raw"].to_numpy(dtype=float),
        )
        scored["usad_ml14_fusion"] = max_score_fusion(scored, ["usad_score", "ml14_fusion"])
        scored["usad_itki_fusion"] = max_score_fusion(scored, ["usad_score", "itki_komutu"])

        test_mask = scored["source_id"].isin(parts["test"]).to_numpy()
        coverage = {
            "split": split_name,
            "val_row_finite_fraction": float(np.isfinite(
                scored.loc[val_mask, "usad_score_raw"].to_numpy(dtype=float)).mean()),
            "test_row_finite_fraction": float(np.isfinite(
                scored.loc[test_mask, "usad_score_raw"].to_numpy(dtype=float)).mean()),
            **window_counts,
        }
        ...
        save_torch_checkpoint(model, models_dir / "usad.pt", metadata={
            "source": SOURCE,
        ...
        (split_dir / "usad_coverage.json").write_text(json.dumps(coverage, indent=2), encoding="utf-8")
    ...
    artifact_manifest = {
        ...
        "stage": "ML-16 Kol U SEAD USAD + official fusion/decision evaluation",
        "source": SOURCE,
        "plan": "docs/ML16_KOL_U_USAD_SEAD_PLAN.md",
        ...
        "usad_window": USAD_WINDOW,
        "usad_stride": USAD_STRIDE,
        "usad_alpha_beta": {"alpha": 0.5, "beta": 0.5},
        "gps_health_column_coverage_note": (
            ...
            "masked_mse/build_windows mask channel absorbs it (see docs/ML16_KOL_U_USAD_SEAD_PLAN.md sec2)."
        ),
...
def main() -> None:
    ...
    print(f"ML-16 Kol U artifact: {output}")
